      ...
      case 3 :
      {
        ...
        do
        {
          ...
          if (not ganador and not perdedor) //revisa las casillas al lado de la nueva posición del jugador.
          {
            ...
            if (queHay == 2)  // hay un tesoro arriba del jugador?
              tesoro.condicionDePaso();
            if (queHay == 3)  // hay un gnomo?
            {
              gnomo.condicionDePaso();
              laberinto.colocaAqui(filaJ -1, columnaJ, 2);
              dispositivo.imprimirMensaje(filaJ +1, columnaJ +1,"♦"); // el gnomo deja un tesoro cuando desaparece.
              dispositivo.imprimirMensaje(filaMensaje, columnaMensaje, " ");
            }
            if (queHay == 4)  // hay un dragón?
          ...
          switch(dispositivo.leerJugada())  // hacia donde se mueve el jugador
          {
            case 1: // arriba
            {
              movimientoVer = -1;
          ...
          switch(queHay)
          {
            ...
            case 4: // Si entra en la casilla del Dragon
            {
              if (dragon.encontrar()) // si le entregas los tesoros, te deja pasar.
              {
                puntosJ += 200;
                laberinto.colocaAqui(filaJ + movimientoVer, columnaJ + movimientoHor, 0);
                dispositivo.imprimirMensaje(filaJ+2, columnaJ+1, " ");
        ...
        // Cuando se acaba el juego muestra tu puntaje y el hall de la fama
        system("clear");
